...
function handleChanges(result: any) {
 ...
 useEffect(() => {
  if (lat !== null && long !== null) {
    Setwload(true); // Show loader when starting to fetch weather data
    getweather(lat, long).then((data: any) => {
      Setweather(data);
      Setwload(false); // Hide loader when data fetching is complete
    }).catch(() => {
      Setwload(false); // Hide loader on error as well
    });
  }
}, [lat, long ]);
